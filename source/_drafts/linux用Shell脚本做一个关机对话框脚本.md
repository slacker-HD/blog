...
用过 Openbox、Fluxbox 这类轻量级窗口管理器的人都清楚，它们的默认菜单里并不包含关机、重启这类系统操作选项。即便手动将这些功能添加到菜单中，使用起来也存在隐患 —— 点击后会直接执行操作，没有任何确认步骤，很容易因误触造成麻烦。为此，我写了一个 Shell 脚本，通过对话框的形式实现操作确认，以此来安全地完成关机、重启、锁屏等功能。
脚本使用yad生成对话框，并使用case语句判断用户点击的按钮，执行相应的命令。我现在用的raspbian的用户直接可以执行`systemctl reboot`等命令，所以就直接运行了，根据权限不同，也可以把自己用户加入`sudoers`等方式，实现无需输入密码来执行关机、重启、锁屏等功能。
